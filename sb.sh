...
hy2port2(){
while true; do
readp "\n设置Hysteria2备用端口 (回车跳过为10000-65535之间的随机端口)：" port
chooseport
if [[ "$port" != "$port_hy2" ]]; then
port_hy2_2=$port
break
fi
yellow "\nHysteria2备用端口不能与主端口相同，请重新设置"
port=
done
}
hy2port3(){
while true; do
readp "\n设置Hysteria2第三端口 (回车跳过为10000-65535之间的随机端口)：" port
chooseport
if [[ "$port" != "$port_hy2" && "$port" != "$port_hy2_2" ]]; then
port_hy2_3=$port
break
fi
yellow "\nHysteria2第三端口不能与已有Hysteria2端口相同，请重新设置"
port=
done
}
hy2port4(){
while true; do
readp "\n设置Hysteria2第四端口 (回车跳过为10000-65535之间的随机端口)：" port
chooseport
if [[ "$port" != "$port_hy2" && "$port" != "$port_hy2_2" && "$port" != "$port_hy2_3" ]]; then
port_hy2_4=$port
break
fi
yellow "\nHysteria2第四端口不能与已有Hysteria2端口相同，请重新设置"
port=
done
}
hy2port5(){
while true; do
readp "\n设置Hysteria2第五端口 (回车跳过为10000-65535之间的随机端口)：" port
chooseport
if [[ "$port" != "$port_hy2" && "$port" != "$port_hy2_2" && "$port" != "$port_hy2_3" && "$port" != "$port_hy2_4" ]]; then
port_hy2_5=$port
break
fi
yellow "\nHysteria2第五端口不能与已有Hysteria2端口相同，请重新设置"
port=
done
}
tu5port(){
...
vlport && vmport && hy2port && hy2port2 && hy2port3 && hy2port4 && hy2port5 && tu5port
if [[ "$sbnh" != "1.10" ]]; then
anport
fi
fi
...
blue "Hysteria-2备用端口：$port_hy2_2"
blue "Hysteria-2第三端口：$port_hy2_3"
blue "Hysteria-2第四端口：$port_hy2_4"
blue "Hysteria-2第五端口：$port_hy2_5"
...
hy2_port2=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[] | select(.tag == "hy2-sb-2") | .listen_port' | head -n 1)
hy2_port3=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[] | select(.tag == "hy2-sb-3") | .listen_port' | head -n 1)
hy2_port4=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[] | select(.tag == "hy2-sb-4") | .listen_port' | head -n 1)
hy2_port5=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[] | select(.tag == "hy2-sb-5") | .listen_port' | head -n 1)
...
hy2_links=("$hy2_link")
if [[ -n $hy2_port2 && "$hy2_port2" != "null" && "$hy2_port2" != "$hy2_port" ]]; then
hy2_link2="hysteria2://$uuid@$sb_hy2_ip:$hy2_port2?security=tls&alpn=h3&insecure=0&allowInsecure=0&sni=$hy2_name&pinSHA256=$SHA256#hy2-2-$hostname"
hy2_links+=("$hy2_link2")
fi
if [[ -n $hy2_port3 && "$hy2_port3" != "null" && "$hy2_port3" != "$hy2_port" && "$hy2_port3" != "$hy2_port2" ]]; then
hy2_link3="hysteria2://$uuid@$sb_hy2_ip:$hy2_port3?security=tls&alpn=h3&insecure=0&allowInsecure=0&sni=$hy2_name&pinSHA256=$SHA256#hy2-3-$hostname"
hy2_links+=("$hy2_link3")
fi
if [[ -n $hy2_port4 && "$hy2_port4" != "null" && "$hy2_port4" != "$hy2_port" && "$hy2_port4" != "$hy2_port2" && "$hy2_port4" != "$hy2_port3" ]]; then
hy2_link4="hysteria2://$uuid@$sb_hy2_ip:$hy2_port4?security=tls&alpn=h3&insecure=0&allowInsecure=0&sni=$hy2_name&pinSHA256=$SHA256#hy2-4-$hostname"
hy2_links+=("$hy2_link4")
fi
if [[ -n $hy2_port5 && "$hy2_port5" != "null" && "$hy2_port5" != "$hy2_port" && "$hy2_port5" != "$hy2_port2" && "$hy2_port5" != "$hy2_port3" && "$hy2_port5" != "$hy2_port4" ]]; then
hy2_link5="hysteria2://$uuid@$sb_hy2_ip:$hy2_port5?security=tls&alpn=h3&insecure=0&allowInsecure=0&sni=$hy2_name&pinSHA256=$SHA256#hy2-5-$hostname"
hy2_links+=("$hy2_link5")
fi
printf "%s\n" "${hy2_links[@]}" > /etc/s-box/hy2.txt
red "🚀【 Hysteria-2 】节点信息如下：" && sleep 2
echo
echo "分享链接【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
for hy2_node in "${hy2_links[@]}"; do
echo -e "${yellow}$hy2_node${plain}"
echo
done
echo
echo "二维码【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
hy2_index=1
for hy2_node in "${hy2_links[@]}"; do
blue "Hysteria-2入口${hy2_index}二维码"
qrencode -o - -t ANSIUTF8 "$hy2_node"
echo
hy2_index=$((hy2_index + 1))
done
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
echo
}
...
for sbfile in $sbfiles; do
[[ -f "$sbfile" ]] || continue
tmpfile=$(mktemp)
if jq --arg cert "$c_c" --arg key "$d_d" '(.inbounds[] | select(.type == "hysteria2") | .tls.certificate_path) = $cert | (.inbounds[] | select(.type == "hysteria2") | .tls.key_path) = $key' "$sbfile" > "$tmpfile"; then
mv "$tmpfile" "$sbfile"
else
rm -f "$tmpfile"
fi
...
[[ -n $hy2_port2 && "$hy2_port2" != "null" && "$hy2_port2" != "$hy2_port" ]] && hy2_showport="$hy2_port/$hy2_port2" || hy2_showport="$hy2_port"
[[ -n $hy2_port3 && "$hy2_port3" != "null" && "$hy2_port3" != "$hy2_port" && "$hy2_port3" != "$hy2_port2" ]] && hy2_showport="$hy2_showport/$hy2_port3"
[[ -n $hy2_port4 && "$hy2_port4" != "null" && "$hy2_port4" != "$hy2_port" && "$hy2_port4" != "$hy2_port2" && "$hy2_port4" != "$hy2_port3" ]] && hy2_showport="$hy2_showport/$hy2_port4"
[[ -n $hy2_port5 && "$hy2_port5" != "null" && "$hy2_port5" != "$hy2_port" && "$hy2_port5" != "$hy2_port2" && "$hy2_port5" != "$hy2_port3" && "$hy2_port5" != "$hy2_port4" ]] && hy2_showport="$hy2_showport/$hy2_port5"
...
green "3：Hysteria2协议 ${yellow}端口:$hy2_showport  转发多端口: $hy2zfport${plain}"
...
echo -e "🚀【  Hysteria-2   】${yellow}端口:$hy2_showport  证书形式:$hy2_zs  转发多端口: $hy2zfport${plain}"
